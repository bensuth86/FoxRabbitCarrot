...
class Fox(Animal):
        ...
        def plot_fox(self):
                ...
                self.prey_consumed = canvas.create_text(x-20,y+20,text=self.prey_consumed,fill = "red")
...
class Population():
        ...
        def hunt(self,animal,rbbt_population):
...
##                animal.prey_consumed = 0
        ...
        def reproduce(self,animal):
                ...
                baby_foxes = [] # store newly created instances to later append to current_population list

                births_pending = int(animal.birthpoints // 1) # animals to be born e.g. 2.5 // 1 = 2 births pending
                animal.birthpoints = animal.birthpoints%1 # remaining birthpoints: 2.5 % 1 = 0.5

                for i in range(0,births_pending):

                        offspring = Animal()
                        #set location based on mother animals x, y coords
                        offspring.x = animal.x * random.choice((-1,1))
                        offspring.y = animal.y * random.choice((-1,1))
                        
                        baby_foxes.append(offspring)
        ...
        def lifecycle(self,rbbt_population):
                ...
                for animal in self.current_population:
                        ...
                        self.move(animal)
                        self.hunt(animal,rbbt_population)
                        baby_foxes = self.reproduce(animal)
                
                print("Current population:", len(self.current_population))
                self.current_population+= baby_foxes                
                print("Current population:", len(self.current_population))
...
##        rbbt_population.lifecycle(carrot_population)
